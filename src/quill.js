...
var EVENTS = ['-shadow-selectionchange', 'mousedown', 'mouseup', 'click'];
var EMITTERS = [];
    ...
    value: function handleDOM(event) {
      ...
      (this.listeners[event.type] || []).forEach(function (_ref) {
        ...
        if (target === node || containsNode(node, target)) {
          if (event.type === '-shadow-selectionchange') {
            // debugger;
          }
...
var _shadowSelectionPolyfill = __webpack_require__(55);

var shadow = _interopRequireWildcard(_shadowSelectionPolyfill);
...
var Selection = function () {
  function Selection(scroll, emitter) {
    ...
    this.handleDragging();
    this.emitter.listenDOM('-shadow-selectionchange', document, function () {
      if (!_this.mouseDown) {
  ...
  _createClass(Selection, [{
    ...
    value: function getNativeRange() {
      ...
      var hasShadowWithNoSelection = this.rootDocument instanceof ShadowRoot && !this.rootDocument.getSelection;
      var hasShadowWithSelection = this.rootDocument instanceof ShadowRoot && this.rootDocument.getSelection;

      if (hasShadowWithNoSelection && isSafari) {
        nativeRange = shadow.getRange(this.rootDocument);
      } else {
        var root = hasShadowWithSelection ? this.rootDocument : document;
...
/* harmony export (immutable) */ __webpack_exports__["getRange"] = getRange;
/* harmony export (immutable) */ __webpack_exports__["internalGetShadowSelection"] = internalGetShadowSelection;
/**
...
const addInternalListener = (() => {
  ...
  document.addEventListener('selectionchange', (ev) => {
    ...
    }, 2); // FIXME: should be bigger than 1 as Quill uses 1
